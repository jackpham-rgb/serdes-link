...
pip install -e .
python -m pytest                # 18 tests: 15 Python (Stage A) + 3 RTL cosims (Stage C)
python scripts/run_link.py      # regenerates every figure in docs/imgs/
```

Stage C's RTL tests also need [Icarus Verilog](http://iverilog.icarus.com)
...
framing: I'm an applied user of these tools for *analysis*, not claiming to
have designed anything with ML. See `docs/00-spec.md`.
